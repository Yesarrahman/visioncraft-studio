...
const Header: React.FC = () => (
  <header className="py-8 px-12 border-b border-slateborder flex justify-between items-center bg-charchar/90 backdrop-blur-2xl sticky top-0 z-50">
    <div className="flex items-center gap-6 group cursor-pointer">
      <div className="w-14 h-14 bg-indigo-500 rounded-2xl flex items-center justify-center font-serif font-bold text-3xl text-white shadow-[0_0_30px_rgba(91,108,255,0.3)] transition-all">V</div>
      <div>
        ...
        <p className="text-[10px] font-black tracking-[0.5em] text-coolgray uppercase opacity-40">Pro Synthesis Dashboard</p>
      </div>
    </div>
    <div className="hidden lg:flex items-center gap-12 text-[11px] font-bold tracking-[0.4em] text-coolgray uppercase">
      <span className="flex items-center gap-3"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> Rendering Core Active</span>
    </div>
  </header>
);
...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (results.length === 0) {
        throw new Error("Neural synthesis failed. Ensure your API_KEY is correctly set in the environment.");
      }
      ...
      console.error(err);
      setError(err.message || "Synthesis disrupted. Check system diagnostics.");
      setAppState(AppState.VIEWING);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-charchar text-softwhite selection:bg-indigo-500/20">
      ...
      <main className="flex-grow max-w-[1300px] mx-auto w-full px-12 py-24 relative">
        {error && (
          <div className="mb-16 bg-red-500/10 border border-red-500/20 p-8 rounded-3xl animate-reveal flex items-start gap-8">
            ...
            <div className="flex-grow">
              <h4 className="font-bold text-red-500 uppercase tracking-[0.4em] text-[10px] mb-2">Diagnostic Error</h4>
              <p className="text-lg text-red-200/70 leading-relaxed font-serif italic">{error}</p>
            </div>
            <button onClick={() => setError(null)} className="text-red-500/40 hover:text-red-500 uppercase text-[9px] font-black tracking-widest mt-2">Dismiss</button>
          </div>
        )}
        ...
        {appState === AppState.GENERATING ? (
          <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-16 animate-reveal">
            ...
            <div className="text-center space-y-6 max-w-lg">
              ...
              <p className="text-[11px] text-coolgray tracking-[0.8em] uppercase font-black opacity-30">Production Synthesis v4.1</p>
            </div>
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-24 items-center">
              ...
              <div className="space-y-16">
                <div className="space-y-6">
                  ...
                  <p className="text-coolgray text-xl font-light leading-relaxed italic">Fine-tune lighting, material properties, and scene details.</p>
                </div>
                <div className="flex flex-wrap gap-3">
                  {["Studio Lighting", "High Polish", "Sunset Glow", "Editorial Focus"].map(tag => (
          ...
          <div className="space-y-64">
            <section className="text-center space-y-20 animate-reveal">
              <div className="inline-block px-8 py-3 rounded-full border border-indigo-500/20 bg-indigo-500/5 text-indigo-400 text-[10px] font-black tracking-[0.5em] uppercase">Professional Rendering Pipeline</div>
              ...
              <p className="text-2xl text-coolgray max-w-2xl mx-auto font-light leading-relaxed opacity-80 italic font-serif">High-fidelity neural rendering for luxury brands and editorial photography.</p>
              ...
              {!selectedImage ? (
                <div className="pt-24">
                  ...
                  <p className="mt-12 text-[11px] text-coolgray uppercase tracking-[0.6em] font-bold opacity-20 italic">Production Ready • Ultra-High Def</p>
                </div>
              ) : (
                  ...
                  {generatedImages.length === 0 && (
                    <div className="w-full max-w-2xl space-y-16">
                      <div className="space-y-6 text-left">
                        <label className="text-[11px] font-black uppercase tracking-[0.5em] text-coolgray/40 ml-8">Campaign brief (Optional)</label>
                      ...
                      <button onClick={generate} className="w-full bg-indigo-500 py-10 rounded-[3rem] font-bold uppercase tracking-[0.6em] text-[14px] shadow-4xl hover:bg-indigo-600 transition-all hover:scale-[1.01] active:scale-99">Process Collection</button>
                    </div>
                  )}
                </div>
              )}
            ...
            {generatedImages.length > 0 && (
              <section className="space-y-32 animate-reveal">
                <div className="flex items-end justify-between border-b border-slateborder pb-16">
                  ...
                  <div className="text-[12px] text-coolgray font-bold tracking-[0.4em] uppercase opacity-30">Neural Collection Generated</div>
                </div>
                    ...
                    <div key={img.id} className="group glass-card rounded-[4.5rem] overflow-hidden hover:shadow-5xl transition-all border border-white/5">
                      <div className="aspect-[3/4] relative overflow-hidden bg-slateborder/20">
                        ...
                        <div className="absolute bottom-14 left-14 right-14 space-y-4">
                           <p className="text-indigo-500 text-[11px] font-black tracking-[0.6em] uppercase">Render Plate</p>
